...
Tinytest.add('html-tools - parseFragment', function (test) {
  ...
  test.throws(function () {
    const _scanner = new Scanner('asdf</p>');
    _scanner.pos = 1;
    HTMLTools.parseFragment(_scanner);
  });
});
...
Tinytest.add('html-tools - getTemplateTag', function (test) {
  ...
  // be anything we want.
  const getTemplateTag = function (scanner) {
    const _scanner = scanner;

    if (!(_scanner.peek() === '{' && // one-char peek is just an optimization
           _scanner.rest().slice(0, 2) === '{{')) return null;

    const match = mustache.exec(_scanner.rest());
    if (!match) _scanner.fatal('Bad mustache');
  ...
  succeed('<a href=http://www.apple.com/>{{foo}}</a>',
          A({ href: 'http://www.apple.com/' }, new TemplateTag({ stuff: 'foo' })));

  // tags not parsed in comments
  succeed('<!--{{foo}}-->', Comment('{{foo}}'));
  succeed('<!--{{foo-->', Comment('{{foo'));

  succeed('&am{{foo}}p;', ['&am', new TemplateTag({ stuff: 'foo' }), 'p;']);
  ...
  succeed('<br {{x}}>', BR(Attrs(new TemplateTag({ stuff: 'x' }))));
  succeed('<br {{x}} {{y}}>', BR(Attrs(new TemplateTag({ stuff: 'x' }),
    new TemplateTag({ stuff: 'y' }))));
  succeed('<br {{x}} y>', BR(Attrs({ y: '' }, new TemplateTag({ stuff: 'x' }))));
  fatal('<br {{x}}y>');
  fatal('<br {{x}}=y>');
  succeed('<br x={{y}} z>', BR({ x: new TemplateTag({ stuff: 'y' }), z: '' }));
  succeed('<br x=y{{z}}w>', BR({ x: ['y', new TemplateTag({ stuff: 'z' }), 'w'] }));
  succeed('<br x="y{{z}}w">', BR({ x: ['y', new TemplateTag({ stuff: 'z' }), 'w'] }));
  succeed('<br x="y {{z}}{{w}} v">', BR({ x: ['y ', new TemplateTag({ stuff: 'z' }),
      new TemplateTag({ stuff: 'w' }), ' v'] }));
  ...
  succeed('<br x={{y}}/>', BR({ x: [new TemplateTag({ stuff: 'y' }), '/'] }));
  succeed('<br x={{z}}{{w}}>', BR({ x: [new TemplateTag({ stuff: 'z' }),
      new TemplateTag({ stuff: 'w' })] }));
  fatal('<br x="y"{{z}}>');
  ...
  succeed('<br {{x 1}}>', BR(Attrs(new TemplateTag({ stuff: 'x 1' }))));
  succeed('<br {{x 1}} {{y 2}}>', BR(Attrs(new TemplateTag({ stuff: 'x 1' }),
    new TemplateTag({ stuff: 'y 2' }))));
  succeed('<br {{x 1}} y>', BR(Attrs({ y: '' }, new TemplateTag({ stuff: 'x 1' }))));
  fatal('<br {{x 1}}y>');
  fatal('<br {{x 1}}=y>');
  succeed('<br x={{y 2}} z>', BR({ x: new TemplateTag({ stuff: 'y 2' }), z: '' }));
  succeed('<br x=y{{z 3}}w>', BR({ x: ['y', new TemplateTag({ stuff: 'z 3' }), 'w'] }));
  succeed('<br x="y{{z 3}}w">', BR({ x: ['y', new TemplateTag({ stuff: 'z 3' }), 'w'] }));
  succeed('<br x="y {{z 3}}{{w 4}} v">', BR({ x: ['y ', new TemplateTag({ stuff: 'z 3' }),
      new TemplateTag({ stuff: 'w 4' }), ' v'] }));
  succeed('<br x={{y 2}}/>', BR({ x: [new TemplateTag({ stuff: 'y 2' }), '/'] }));
  succeed('<br x={{z 3}}{{w 4}}>', BR({ x: [new TemplateTag({ stuff: 'z 3' }),
      new TemplateTag({ stuff: 'w 4' })] }));

  succeed('<p></p>', P());

  succeed('x{{foo}}{{bar}}y', ['x', new TemplateTag({ stuff: 'foo' }),
    new TemplateTag({ stuff: 'bar' }), 'y']);
  succeed('x{{!foo}}{{!bar}}y', 'xy');
  succeed('x{{!foo}}{{bar}}y', ['x', new TemplateTag({ stuff: 'bar' }), 'y']);
  succeed('x{{foo}}{{!bar}}y', ['x', new TemplateTag({ stuff: 'foo' }), 'y']);
  ...
  succeed('<textarea {{a}} x=1 {{b}}></textarea>',
          TEXTAREA(Attrs({ x: '1' }, new TemplateTag({ stuff: 'a' }),
                         new TemplateTag({ stuff: 'b' }))));
});
