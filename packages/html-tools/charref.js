...
(function () {
  ...
  Object.getOwnPropertyNames(namedEntitiesByFirstChar).forEach((chr) => {
...
const getNamedCharRef = function (scanner, inAttribute) {
  ...
  if (!peekMatcher(_scanner, getPossibleNamedEntityStart)) return null;

  const matcher = getNamedEntityByFirstChar[_scanner.rest().charAt(1)];
  let entity = null;
  if (matcher) entity = peekMatcher(_scanner, matcher);
  ...
  if (entity) {
    if (entity.slice(-1) !== ';') {
      ...
      if (inAttribute && ALPHANUMERIC.test(_scanner.rest().charAt(entity.length))) return null;
      _scanner.fatal(`Character reference requires semicolon: ${entity}`);
      return null;
    }
      _scanner.pos += entity.length;
      return entity;
  }
    // we couldn't match any real entity, so see if this is a bad entity
    // or something we can overlook.
    const badEntity = peekMatcher(_scanner, getApparentNamedEntity);
    if (badEntity) _scanner.fatal(`Invalid character reference: ${badEntity}`);
...
const BIG_BAD_CODEPOINTS = (function (obj) {
  ...
  const list = [0x1FFFE, 0x1FFFF, 0x2FFFE, 0x2FFFF, 0x3FFFE, 0x3FFFF,
              ...
              0x10FFFE, 0x10FFFF];
  for (let i = 0; i < list.length; i++) _obj[list[i]] = true;
...
export function getCharacterReference (scanner, inAttribute, allowedChar) {
  ...
  // no ampersand
  if (_scanner.peek() !== '&') { return null; }

  const afterAmp = _scanner.rest().charAt(1);

  if (afterAmp === '#') {
    _scanner.pos += 2;
    // refNumber includes possible initial `x` and final semicolon
    const refNumber = getCharRefNumber(_scanner);
    // At this point we've consumed the input, so we're committed to returning
    // something or failing fatally.
    if (!refNumber) _scanner.fatal('Invalid numerical character reference starting with &#');
    let codepoint;
    if (refNumber.charAt(0) === 'x' || refNumber.charAt(0) === 'X') {
      // hex
      let hex = refNumber.slice(1, -1);
      while (hex.charAt(0) === '0') hex = hex.slice(1);
      if (hex.length > 6) _scanner.fatal(`Numerical character reference too large: 0x${hex}`);
      codepoint = parseInt(hex || '0', 16);
    } else {
      let dec = refNumber.slice(0, -1);
      while (dec.charAt(0) === '0') dec = dec.slice(1);
      if (dec.length > 7) _scanner.fatal(`Numerical character reference too large: ${dec}`);
      codepoint = parseInt(dec || '0', 10);
    }
    if (!isLegalCodepoint(codepoint)) _scanner.fatal(`Illegal codepoint in numerical character reference: &#${refNumber}`);
    return { t: 'CharRef',
             v: `&#${refNumber}`,
             cp: [codepoint] };
  } if ((!afterAmp) // EOF
    ...
    const namedEntity = getNamedCharRef(_scanner, inAttribute);
    if (namedEntity) {
      return { t: 'CharRef',
               v: namedEntity,
