...
export function parseIdentifierName(scanner) {
  ...
  return _scanner.input.substring(startPos, _scanner.pos);
}
...
export function parseExtendedIdentifierName(scanner) {
  ...
  if (_scanner.peek() === '@') {
    _scanner.pos++;
    const afterAt = parseIdentifierName(_scanner);
    if (afterAt) {
      return `@${afterAt}`;
    }
    _scanner.pos--;
  ...
  return parseIdentifierName(_scanner);
}
...
export function parseStringLiteral(scanner) {
  ...
  while (match) {
    ...
    if (match) {
      ...
      if (match) {
        ...
        // can use JSON.parse for some work.  JSON strings are not the
  ...
  if (!match || match[0] !== quote) _scanner.fatal('Unterminated string literal');

  jsonLiteral += '"';
  const text = _scanner.input.substring(startPos, _scanner.pos);
