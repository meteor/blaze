...
export function flattenAttributes (attrs) {
  ...
  for (let i = 0, N = (isList ? attrs.length : 1); i < N; i++) {
    ...
    if (oneAttrs) {
      Object.getOwnPropertyNames(oneAttrs).forEach((name) => {
...
export function CharRef(attrs) {
  // called without `new`
  if (!(this instanceof CharRef)) { return new CharRef(attrs); }
...
export function Comment(value) {
  // called without `new`
  if (!(this instanceof Comment)) { return new Comment(value); }
...
export function Raw(value) {
  // called without `new`
  if (!(this instanceof Raw)) { return new Raw(value); }
