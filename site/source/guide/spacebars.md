...
As previously mentioned, Spacebars is a Meteor template language inspired by
...
This page intends to provide deeper insight on the Spacebars syntax and how
to leverage it to build reactive HTML templates.

## Getting Started

A Spacebars template consists of HTML interspersed with template tags, which are
delimited by <code v-pre>{{</code> and <code v-pre>}}</code> (two curly braces).
